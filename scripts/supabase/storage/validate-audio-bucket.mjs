...
/**
 * Audio Bucket Validation Script
 * Validates that the private 'audio' bucket exists and has proper configuration
 ...
 */
async function checkStorageHealth(supabase, bucket = 'audio') {
  try {
    ...
    const healthCheckPath = `audio/healthcheck/health_${Date.now()}.mp3`
...
async function validateAudioBucket() {
  console.log('🔍 Validating audio bucket configuration...')

  try {
    // Run health check
    const healthResult = await checkStorageHealth(supabase, 'audio')

    if (!healthResult.healthy) {
      console.error('❌ Audio bucket health check failed:', healthResult.message)
    ...
    console.log('✅ Audio bucket health check passed:', healthResult.message)
    ...
    console.log('📦 Audio bucket details:')
    ...
    console.log(`   Allowed MIME types: ${audioBucket.allowed_mime_types?.join(', ') || 'none'}`)
    ...
    if (audioBucket.file_size_limit !== 52428800) { // 50 MiB
      issues.push(`File size limit should be 52428800 bytes (50 MiB), got ${audioBucket.file_size_limit}`)
    }

    const expectedMimes = ['audio/mpeg', 'audio/wav', 'audio/x-wav', 'audio/wave']
    const actualMimes = audioBucket.allowed_mime_types || []
    ...
    console.log('✅ Audio bucket configuration is correct')
    ...
    try {
      ...
      const { data: triggerData, error: triggerError } = await supabase.rpc('sql', {
        query: `
          ...
          WHERE trigger_name = 'trg_audio_mime_whitelist'
      ...
      if (!triggerData || triggerData.length === 0) {
        console.error('❌ Audio MIME validation trigger not found')
        process.exit(1)
      }
      ...
      const { data: policyData, error: policyError } = await supabase.rpc('sql', {
        query: `
          ...
          AND policyname = 'Service role can manage audio files'
        `
      })
      ...
      if (!policyData || policyData.length === 0) {
        console.error('❌ Audio bucket RLS policy not found')
        process.exit(1)
      }
      ...
      const invalidTestPath = 'audio/test/invalid.png'
    ...
    const testPath = 'audio/test/test.mp3'
    ...
    console.log('🎉 Audio bucket validation completed successfully!')
    console.log('')
    console.log('Summary:')
