-- Rename text_feedback_text column to full_feedback_text in analysis_jobs table
-- This change aligns the column name with the updated naming convention

-- Rename the column
alter table public.analysis_jobs
rename column text_feedback_text to full_feedback_text;

-- Update the comment to reflect the new naming
comment on column public.analysis_jobs.full_feedback_text is 'Complete AI-generated feedback text';

-- Update function parameter and internal references
drop function if exists public.store_enhanced_analysis_results(bigint, text, text, text, text, bigint, text, jsonb, jsonb);
create function public.store_enhanced_analysis_results(
  analysis_job_id bigint,
  p_full_feedback_text text default null, -- Keep parameter name for backward compatibility
...
begin
  ...
  set
    full_feedback_text = coalesce(p_full_feedback_text, full_feedback_text), -- Updated column reference
...
-- Update the get_enhanced_analysis_with_feedback function to return full_feedback_text
...
returns table (
  ...
  full_feedback_text text, -- Updated column name in return type
  summary_text text,
  ssml text,
  audio_url text,
...
begin
  ...
  if (select auth.role()) = 'service_role' then
    ...
    select
      ...
      aj.full_feedback_text, -- Updated column reference
      aj.summary_text,
      aj.ssml,
      aj.audio_url,
    ...
    group by aj.id, aj.status, aj.progress_percentage, aj.full_feedback_text, aj.summary_text, -- Updated column reference
