...
-- Storage schema functions are managed by Supabase and should not be recreated
-- All storage.* functions and triggers are commented out below
/*
...
-- Storage schema tables are managed by Supabase and should not be recreated
-- All storage.* CREATE/ALTER statements below are commented out with --
-- Storage schema is managed by Supabase platform, we only create buckets via INSERT in storage migration
...
-- 
-- 
-- -- Storage trigger commented out (managed by Supabase)
...
-- Enable realtime for tables (idempotent - skip if tables don't exist yet)
DO $$
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'analysis_jobs') THEN
    BEGIN
      ALTER PUBLICATION supabase_realtime ADD TABLE analysis_jobs;
...
DO $$
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'video_recordings') THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE video_recordings;
  END IF;
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'analysis_feedback') THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE analysis_feedback;
  END IF;
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
