...
// Mock logger
vi.mock('@my/ui/src/utils/logger', () => ({
  log: {
    info: vi.fn(),
    warn: vi.fn(),
    error: vi.fn(),
...
describe('useScreenStateTransition', () => {
  const defaultProps = {
    onNavigateToVideoPlayer: vi.fn(),
    onNavigateToCamera: vi.fn(),
  }
  ...
  describe('State Transitions', () => {
    it('should transition to video player when recording stops', () => {
      const { result } = renderHook(() => useScreenStateTransition(defaultProps))
    ...
    it('should transition back to camera when restart is requested', () => {
      const { result } = renderHook(() => useScreenStateTransition(defaultProps))
    ...
    it('should transition back to camera when continue to analysis is requested', () => {
      const { result } = renderHook(() => useScreenStateTransition(defaultProps))
  ...
  describe('Video Data Management', () => {
    it('should store video data when transitioning to video player', () => {
      const { result } = renderHook(() => useScreenStateTransition(defaultProps))
    ...
    it('should clear video data when transitioning back to camera', () => {
      const { result } = renderHook(() => useScreenStateTransition(defaultProps))
  ...
  describe('Callback Integration', () => {
    it('should call onNavigateToVideoPlayer with correct data', () => {
      const onNavigateToVideoPlayer = vi.fn()
    ...
    it('should call onNavigateToCamera when returning to camera', () => {
      const onNavigateToCamera = vi.fn()
      const { result } = renderHook(() =>
        useScreenStateTransition({
          ...defaultProps,
          onNavigateToCamera,
