...
// Enhanced performance monitoring interface from native pipeline
/**
 * MVP Performance Metrics - Simplified for basic FPS tracking
 * Advanced metrics (thermal, battery, memory) preserved for production upgrade
 */
export interface PerformanceMetrics {
  fps: number // ✅ KEEP - Basic FPS tracking
  processingTime: number // ✅ KEEP - Simple timing
  averageProcessingTime: number // ✅ KEEP - Basic average
  // ❌ REMOVED for MVP: thermalState, batteryLevel, memoryUsage, droppedFrames, frameWidth, frameHeight, timestamp
}
...
export interface CameraRecordingStore {
  ...
  // Performance monitoring (from native pipeline)
  ...
  // Performance monitoring actions (from native pipeline)
...
/**
 * MVP Performance Metrics - Simplified initial values
 */
const initialPerformanceMetrics: PerformanceMetrics = {
  fps: 30, // ✅ Basic FPS tracking
  processingTime: 0, // ✅ Simple timing
  averageProcessingTime: 0, // ✅ Basic average
}
...
export const useCameraRecordingStore = create<CameraRecordingStore>()(
  subscribeWithSelector(
    immer((set, get) => ({
      ...
      // MVP Performance monitoring - Simplified for basic FPS tracking
      updatePerformanceMetrics: (updates) =>
        set((draft) => {
          Object.assign(draft.performance, updates)

          // Calculate running average for processing time (MVP simplified)
          if (updates.processingTime !== undefined) {
            const currentAvg = draft.performance.averageProcessingTime
            const newProcessingTime = updates.processingTime
            draft.performance.averageProcessingTime =
              currentAvg === 0 ? newProcessingTime : (currentAvg + newProcessingTime) / 2
          }
        }),

      resetPerformanceMetrics: () =>
        set((draft) => {
          draft.performance = { ...initialPerformanceMetrics }
        }),
