...
export async function validateVideoFile(
  ...
  if (!opts.allowedFormats.includes(file.type)) {
    errors.push(
      `Invalid format. Supported formats: ${opts.allowedFormats.join(", ")}`,
    );
  }
  ...
  if (file.size > opts.maxFileSizeBytes) {
    errors.push(
      `File too large. Maximum size: ${formatFileSize(opts.maxFileSizeBytes)}`,
    );
  }
  ...
  try {
    ...
    if (metadata.duration > opts.maxDurationSeconds) {
      errors.push(
        `Video too long. Maximum duration: ${opts.maxDurationSeconds} seconds`,
      );
    }

    if (metadata.duration < opts.minDurationSeconds) {
      errors.push(
        `Video too short. Minimum duration: ${opts.minDurationSeconds} seconds`,
      );
    }
    ...
    if (metadata.width && metadata.height) {
      ...
      if (
        width < opts.minResolution.width || height < opts.minResolution.height
      ) {
        errors.push(
      ...
      if (
        width > opts.maxResolution.width || height > opts.maxResolution.height
      ) {
        warnings.push(
          `Video resolution very high. May cause performance issues.`,
        );
      }
      ...
      if (aspectRatio < 0.5 || aspectRatio > 2.0) {
        warnings.push(
          "Video has unusual aspect ratio. Square or 16:9 recommended.",
        );
      }
    }
    ...
    if (bitrate < 500000) {
      warnings.push(
        "Video quality appears to be very low. Consider using higher bitrate.",
      );
    } else if (bitrate > 10000000) {
      warnings.push(
        "Video bitrate is very high. File size may be unnecessarily large.",
      );
    }
    ...
    if (file.type === "video/quicktime" || file.type === "video/mov") {
      warnings.push(
        "MOV format detected. MP4 recommended for better compatibility.",
      );
    }
  } catch (_error) {
    errors.push(
      "Unable to read video metadata. File may be corrupted or unsupported format.",
    );
  }
...
export function isVideoFile(file: File): boolean {
  return file.type.startsWith("video/") &&
    DEFAULT_OPTIONS.allowedFormats.includes(file.type);
}
